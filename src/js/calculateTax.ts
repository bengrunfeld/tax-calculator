...
const calcTaxableIncome = (totalRevenue: number, totalDeductions: number) =>
  totalRevenue - totalDeductions

// 4. Apply the company tax rate to taxable income
const calcCompanyTaxRate = (taxableIncome: number, corpTaxRate: number) =>
  taxableIncome * corpTaxRate

// 5. Calculate the medicare levy and add it to total tax owed
const calcMedicareLevy = (totalCompanyTax: number, medicareRate: number) =>
  totalCompanyTax * medicareRate

// 6. Output the result as total tax owed
const calcTotalTaxOwed = (totalCompanyTax: number, medicareLevy: number) =>
